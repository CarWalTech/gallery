...
class AlbumsApi {
  ...
  /// Get child albums
  ///
  /// Retrieve the direct child (sub) albums of a specific album.
  ///
  /// Note: This method returns the HTTP [Response].
  ///
  /// Parameters:
  ///
  /// * [String] id (required):
  Future<Response> getChildAlbumsWithHttpInfo(String id,) async {
    // ignore: prefer_const_declarations
    final apiPath = r'/albums/{id}/children'
      .replaceAll('{id}', id);
  ...
  ///
  /// * [String] id (required):
  Future<List<AlbumResponseDto>?> getChildAlbums(String id,) async {
    final response = await getChildAlbumsWithHttpInfo(id,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
