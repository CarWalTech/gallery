<script lang="ts">
  ...
  const buildBreadcrumbs = async () => {
    ...
    while (currentAlbum?.parentId) {
      try {
        const parent = await getAlbumInfo({ id: currentAlbum.parentId });
...
<nav class="flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400 mb-2" aria-label="Breadcrumb">
  <a href={Route.albums()} class="hover:text-immich-primary dark:hover:text-immich-dark-primary transition-colors">
    {$t('albums') ?? 'Albums'}
  </a>
